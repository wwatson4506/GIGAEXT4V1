 ...
 *
 * EXT4FileSystem.h adapted from FATFileSystem.h
 * Copyright (c) 2024, Warren Watson.
 */

#ifndef EXT4FILESYSTEM_H
#define EXT4FILESYSTEM_H
...
#define DONT_SHOW_DOT_FILES 1 // Set to one to hide dot files.
...
public:
    ...
    EXT4FileSystem(const char *name = NULL, BlockDevice *bd = NULL);
    virtual ~EXT4FileSystem();
    
	// These may be temporary...
	virtual GIGAext4 *getfs(void) { return &_fs; }
    ...
    virtual const char *getVolumeLabel(void);
...
private:
    GIGAext4 _fs; // Work area (file system object) for logical drive.
    char _fsid[sizeof("sda1")];
    int _id; // Partition ID
	struct ext4_mount_stats stats;
    uint8_t partition_type = 0;
};

//using mbed::EXT4FileSystem;
